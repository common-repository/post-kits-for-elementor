...
for ($i = 0; $i < $slider_items; $i++) {
    ...
    $output .= '<div class="alith-layout alith-module-layout-11 alith-block-'.esc_attr($rand) .esc_attr((empty($atts['alith_block_post_custom_class']) ? '' : $atts['alith_block_post_custom_class'])).'">
        ...
        while ($the_query->have_posts() && $post_count <= $length) {
            ...
            if ($post_count == 1) {
                $output .= '<div class="row">
                        <article class="col-lg-4 col-md-6 col-sm-12 mb-sm-30 mb-md-0 alith-custom-padding-right">
                            ...
                            $output .= alith_meta_output($atts, true, true, false, true);
                ...
                if ($post_count <=6) {
                    $output .= '<article class="alith-epk-row row mb-lg-15 mb-md-0 mb-sm-15 alith-custom-padding-bottom post-small">
                                    ...
                                    <div class="col-md-8 col-sm-6 pl-md-0">
                                        <h5 class="alith_epk_title alith_epk_second_title alith-second-font-size row-limited-2">
                                            <a ' . alith_bp_title_link_effect($atts) . ' href="'.esc_url(get_permalink()).'">'.esc_html(get_the_title()).'</a>
                                        </h5>';
                    $output .=          '<div class="d-lg-block d-none">'.alith_meta_output($atts, false, true, true, false).'</div>';
                    $output .='     </div>
                                </article>';
                    if ($post_count == 6) { $output .='</div><div class="col-lg-4 d-none d-lg-block">';}
                } else {
                    $output .= '<article class="alith-epk-row row mb-lg-15 mb-sm-15 alith-custom-padding-bottom post-small">
                                    <div class="col-md-4 col-sm-6 mb-sm-15 mb-lg-0">
                                        <figure class="alith_epk_thumbs hover-opacity position-relative mb-lg-0 zoom custom-rounded">
                                            <a href="'.esc_url(get_permalink()).'">
                                                <img clas="custom-rounded" src="'.$image_src.'">
