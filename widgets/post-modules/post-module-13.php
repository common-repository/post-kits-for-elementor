...
for ($i = 0; $i < $slider_items; $i++) {
    ...
    $output .= '<div class="alith-layout alith-module-layout-13 alith-block-'.esc_attr($rand) .esc_attr((empty($atts['alith_block_post_custom_class']) ? '' : $atts['alith_block_post_custom_class'])).'">
    ...
    if ($the_query->have_posts()  && $all_count_post >= $posts_capacity) {
        ...
        while ($the_query->have_posts() && $post_count <= $length) {
            ...
            if ($post_count <=2) {
                $output .= '
                    <article class="col-md-6 alith-custom-padding-right mb-sm-15">
                        ...
                        $output .= ' <a class="alith-epk-read-more mb-lg-15 mt-15 mt-md-0" href="' . esc_url(get_permalink()) . '">' . esc_html($atts['alith_block_post_read_more_text'],'alith_epk') . '</a>';
                    }
                $output .= '
                    </article>';
            }            
            else {              
               $output .= '<article class="col-lg-4 col-md-6 col-sm-12 mb-sm-30">
                                <div class="custom-rounded mb-sm-15">
