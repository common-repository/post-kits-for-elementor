...
for ($i = 0; $i < $slider_items; $i++) {
    ...
    $output .= '<div class="alith-layout alith-module-layout-7 alith-block-'.esc_attr($rand) .esc_attr((empty($atts['alith_block_post_custom_class']) ? '' : $atts['alith_block_post_custom_class'])).'">
    ...
    if ($the_query->have_posts()  && $all_count_post >= $posts_capacity) {
        ...
        while ($the_query->have_posts() && $post_count <= $length) {
            ...
            $output .= '<article class="col-sm-12 mb-sm-30">
                            <div class=" alith-epk-row row">
                                ...
                                <div class="col-lg-9 col-md-8 col-sm-6 pl-md-0">                                    
                                    ...
                                    $output .= '<p class="alith-epk-excerpt mt-15 mb-0">
                                         '.esc_html(alith_epk_custom_excerpt(esc_attr($excerpt_length))).'
                                    </p>';
                                    if ($atts['alith_block_post_read_more_text'] != '') {
                                        $output .= ' <a class="alith-epk-read-more mt-sm-15 d-md-none d-sm-inline-block d-lg-inline-block custom-rounded" href="' . esc_url(get_permalink()) . '">' . esc_html($atts['alith_block_post_read_more_text'],'alith_epk') . '</a>';
                                    }
