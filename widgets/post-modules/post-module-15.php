...
for ($i = 0; $i < $slider_items; $i++) {
    ...
    $output .= '<div class="alith-layout alith-module-layout-15 alith-block-'.esc_attr($rand) .esc_attr((empty($atts['alith_block_post_custom_class']) ? '' : $atts['alith_block_post_custom_class'])).'">
    ...
    if ($the_query->have_posts()  && $all_count_post >= $posts_capacity) {
        ...
        while ($the_query->have_posts() && $post_count <= $length) {
            ...
            if ($post_count == 1) {
                $output .= '
                        <article class="alith-epk-row row alith-custom-padding-right alith-epk-mb-15">
                ...
                $output .= '<article class="col-lg-3 col-md-4 col-sm-12 mb-sm-30">
                                ...
                                <h5 class="alith_epk_title alith-second-font-size row-limited-2">
                                    <a ' . alith_bp_title_link_effect($atts) . ' href="'.esc_url(get_permalink()).'">'.esc_html(get_the_title()).'</a>
                                </h5>';
