...
for ($i = 0; $i < $slider_items; $i++) {
    ...
    $output .= '<div class="alith-layout alith-aside-layout-1 alith-block-'.esc_attr($rand) .esc_attr((empty($atts['alith_block_post_custom_class']) ? '' : $atts['alith_block_post_custom_class'])).'">
            ...
            if ($post_count == 1) {
                $output .= '<article class="col-sm-12 item-space-bottom">
                            ...
                            if ($atts['alith_block_post_read_more_text'] != '') {
                                $output .= ' <a class="alith-epk-read-more d-md-none d-sm-inline-block d-lg-inline-block" href="' . esc_url(get_permalink()) . '">' . esc_html($atts['alith_block_post_read_more_text'],'alith_epk') . '</a>';
                            }                                    
                $output .= '</article>';
            } else {
                $output .= '<article class="col-sm-12 item-space-bottom">
                            <div class=" alith-epk-row row">
                                <div class="col-md-4 col-sm-4 col-xs-4">
                                    <figure class="alith_epk_thumbs hover-opacity position-relative mb-lg-0 mb-sm-15 zoom custom-rounded">
                                        <a ' . alith_bp_title_link_effect($atts) . ' href="'.esc_url(get_permalink()).'">
                                            <img clas="custom-rounded" src="'.$image_src_thumbnail.'">
                                        </a>                               
                                    </figure>
                                </div>
                                <div class="col-md-8 col-sm-8 col-xs-8 pl-0">                                    
                                    ...
                                    $output .= alith_meta_output($atts, true, true, false, false);             
                 $output .=     '</div>
                            </div>
                        </article>';
            }
