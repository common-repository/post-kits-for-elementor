...
for ($i = 0; $i < $slider_items; $i++) {
    ...
    $output .= '<div class="alith-layout alith-grid-layout-1 alith-block-'.esc_attr($rand) .esc_attr((empty($atts['alith_block_post_custom_class']) ? '' : $atts['alith_block_post_custom_class'])).'">
    ...
    if ($the_query->have_posts()  && $all_count_post >= $posts_capacity) {
        ...
        while ($the_query->have_posts() && $post_count <= $length) {
            ...
            switch ($post_count) {
                case 1:
                    $output .='<div class="alith-width-1-4 alith-grid">';
                    $output .=  '<div class="alith-width-1">';
                    $output .=      alith_grid_small_post_render($atts, 'item-space-right-2 item-space-bottom-2', $image_src);
                    $output .=  '</div>';
                    break;
                case 2:
                    $output .=  '<div class="alith-width-1">';                   
                    ...
                    break;
                case 3:                    
                    $output .= alith_grid_big_post_render($atts, 'item-space-left-2 item-space-right-2', $image_src);
                    $output .='</div>';
                    break;
                case 4:
                    $output .='<div class="alith-width-1-4 alith-grid">';
