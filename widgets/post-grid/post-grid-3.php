...
for ($i = 0; $i < $slider_items; $i++) {
    ...
    $output .= '<div class="alith-layout alith-grid-layout-3 alith-block-'.esc_attr($rand) .esc_attr((empty($atts['alith_block_post_custom_class']) ? '' : $atts['alith_block_post_custom_class'])).'">
        ...
        while ($the_query->have_posts() && $post_count <= $length) {
            ...
            switch ($post_count) {
                case 1:
                    ...
                    $output .='</div>';
                    $output .='<div class="alith-width-1-2 alith-grid">';
                    break;
                case 2:
                ...
                case 3: 
                    $output .=      '<div class="alith-width-1">';              
                    $output .=          alith_grid_small_post_render($atts, 'item-space-top-2', $image_src);
                    $output .=      '</div>';
                    $output .='</div>';
                    break;
                default:
