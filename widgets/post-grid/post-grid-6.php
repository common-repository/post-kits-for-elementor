...
for ($i = 0; $i < $slider_items; $i++) {
    ...
    $output .= '<div class="alith-layout alith-grid-layout-6 alith-block-'.esc_attr($rand) .esc_attr((empty($atts['alith_block_post_custom_class']) ? '' : $atts['alith_block_post_custom_class'])).'">
        ...
        while ($the_query->have_posts() && $post_count <= $length) {
            ...
            switch ($post_count) {
                case 1:                   
                    ...
                    $output .=      alith_grid_big_post_render($atts, 'item-space-bottom-2', $image_src);
                    $output .=  '</div>';
                    break;
                case 2:
                    $output .=  '<div class="alith-width-1 item-space-top-2">';                   
