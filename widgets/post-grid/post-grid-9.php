...
for ($i = 0; $i < $slider_items; $i++) {
    ...
    $output .= '<div class="alith-layout alith-grid-layout-9 alith-block-'.esc_attr($rand) .esc_attr((empty($atts['alith_block_post_custom_class']) ? '' : $atts['alith_block_post_custom_class'])).'">
    ...
    if ($the_query->have_posts()  && $all_count_post >= $posts_capacity) {
        ...
        while ($the_query->have_posts() && $post_count <= $length) {
            ...
            switch ($post_count) {
                case 1:                   
                    $output .=  '<div class="alith-width-1 alith-grid">';
                ...
                case 2:                   
                    $output .=      '<div class="alith-width-1-2">';
                    $output .=          alith_grid_big_post_render($atts, 'item-space-bottom-2 item-space-left-2', $image_src);
                    $output .=      '</div>';
                    $output .=  '</div>';
                    break;               
                case 3:
