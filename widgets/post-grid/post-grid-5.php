...
for ($i = 0; $i < $slider_items; $i++) {
    ...
    $output .= '<div class="alith-layout alith-grid-layout-5 alith-block-'.esc_attr($rand) .esc_attr((empty($atts['alith_block_post_custom_class']) ? '' : $atts['alith_block_post_custom_class'])).'">
    ...
    if ($the_query->have_posts()  && $all_count_post >= $posts_capacity) {
        ...
        while ($the_query->have_posts() && $post_count <= $length) {
            ...
            switch ($post_count) {
                case 1:
                    $output .='<div class="alith-width-3-4">';                    
